...
const generatedControllers = {
  ...
  init: router => {
    ...
    router.get(baseUrl + "/findByuser_accounts/:key", authorize([]), UserController.findByuser_accounts);
  ...
  */
  ...
  /**
  * UserModel.findByuser_accounts
  *   @description CRUD ACTION findByuser_accounts
  *   @param Objectid key Id of the resource user_accounts to search
  *
  */
  findByuser_accounts: async (req, res) => {
    try {
      const result = await UserModel.findByuser_accounts(req.params.key);
      res.json(result);
    } catch (err) {
      const safeErr = ErrorManager.getSafeError(err);
      res.status(safeErr.status).json(safeErr);
